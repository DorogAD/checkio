...
def vacation(start_date, days):
    work_day = date(int(start_date.split('-')[0]), int(start_date.split('-')[1]),
                    int(start_date.split('-')[2])) + timedelta(days=days)
    return str(work_day + timedelta(days=7 - work_day.weekday())) if work_day.weekday() >= 5 else str(work_day)
