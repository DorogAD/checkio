...
The task is to calculate one of the permutations of the consecutive integers (starting from 0) and return it
...
def reversed_permutation_index(length: int, index: int) -> Iterable[int]:
    for number, element in enumerate(permutations(range(length))):
        if number + 1 == index:
            return element
