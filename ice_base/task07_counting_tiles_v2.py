...
def checkio(radius: float) -> list:
    """count tiles"""
    whole, partial = 0, 0
    for coord_x in range(int(radius // 1 + 1)):
        for coord_y in range(int(radius // 1 + 1)):
            if ((coord_x + 1) ** 2 + (coord_y + 1) ** 2) ** 0.5 < radius:
                whole += 4
            if (coord_x ** 2 + coord_y ** 2) ** 0.5 < radius:
                partial += 4
    return [whole, partial - whole]
