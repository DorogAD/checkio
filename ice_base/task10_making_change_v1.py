...
def checkio(price: int, denominations: list) -> int:
    ...
    for number in denominations:
        maximum = price
        while maximum - number >= 0:
            all_list.append(number)
            maximum -= number
    min_coin = price
    for i in range(1, int(price / 2)):
        for element in permutations(all_list, i):
            if sum(element) == price and len(element) < min_coin:
                min_coin = len(element)
