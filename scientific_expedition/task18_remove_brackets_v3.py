...
def count_and_cut(expression: str, br1: str, br2: str) -> str:
    br1_count = expression.count(br1)
    br2_count = expression.count(br2)
    if br1_count > br2_count:
        expression = expression.replace(br1 * (br1_count - br2_count), '', 1)
    elif br1_count < br2_count:
        expression = expression.replace(br2 * (br2_count - br1_count), '', 1)
    return expression


def remove_brackets(line: str) -> str:
    return count_and_cut(count_and_cut(count_and_cut(re.sub('\(.{1}\)', r'()', line), '{', '}'), '[', ']'), '(', ')')
