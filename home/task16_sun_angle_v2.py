"""
Every true traveler must know how to do 3 things: fix the fire, find the water and extract useful information from the
nature around him. Programming won't help you with the fire and water, but when it comes to the information extraction -
...
def sun_angle(time):
    now = datetime.timedelta(hours=int(time[:2]), minutes=int(time[3:]))
    hour_6 = datetime.timedelta(hours=6)
    hour_18 = datetime.timedelta(hours=18)
    if now.seconds < hour_6.seconds or hour_18.seconds < now.seconds:
        return "I don't see the sun!"
    else:
        return (now.seconds - hour_6.seconds) / 240
